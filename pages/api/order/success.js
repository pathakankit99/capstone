       ...
       Order.create(arr, (err, order) => {
         if (err) {
          ...
          //  console.log(order,'order')
           ...
           body.cart.map((item) => {
            Dish.findOne({ _id: item._id }).exec(function (
               err,
               updatedDish
             ) {
               if (err) {
               ...
               updatedDish.itemsSold = updatedDish.itemsSold + item.quantity
               Dish.updateOne(
                 { _id: item._id },
                 updatedDish,
                 function (err, party) {
                   if (err) {
                     console.log(err,'error in updating dish itemsSold ')
                    //  res.status(400)
                    //  res.send(err)
                   }
                   console.log('updated dish itemssold')
                  //  res.json({ data: order })
                 }
               )
             })
